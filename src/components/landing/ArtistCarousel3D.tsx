...
export default function ArtistCarousel3D() {
  ...
  // Update active dot based on angle
        ...
        >
          {ARTISTS.map((a, i) => {
            ...
            return (
              ...
              >
                <ArtistCard artist={a} cardAngle={cardAngle} totalAngle={angle} />
              </div>
            );
          })}
        </motion.div>
 ...
 * ========================================================================= */
const ArtistCard = memo(({ artist, cardAngle, totalAngle }: { artist: Artist; cardAngle: number; totalAngle: MotionValue<number> }) => {
  const accent = artist.accent || '#FF4D00';

  // Compute facing factor — front-most card scales / glows brighter
  const relative = useTransform(totalAngle, (a) => ((cardAngle + a) % 360 + 360) % 360);
  const distFromFront = useTransform(relative, (r) => Math.min(r, 360 - r));

  const scale = useTransform(distFromFront, [0, 30, 180], [1.05, 1, 1]);
  ...
  const liveBadgeOpacity = useTransform(distFromFront, [0, 30], [1, 0]);
