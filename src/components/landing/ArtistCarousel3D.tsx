...
export default function ArtistCarousel3D() {
  // ⚡ Bolt: Use MotionValue for angle to bypass React re-renders (60fps -> 0 React renders for rotation)
  const angle = useMotionValue(0);
  const [paused, setPaused] = useState(false);

  // ⚡ Bolt: Use a separate state for pagination that only updates when the active card changes
  ...
  // ⚡ Bolt: Optimized animation loop using Framer Motion's internal loop
  ...
  // ⚡ Bolt: Only update active index when it actually changes to minimize pagination re-renders
  useMotionValueEvent(angle, "change", (latest) => {
    const currentActive = Math.round((360 - latest) / stepDeg) % cardCount;
    const normalizedActive = (currentActive + cardCount) % cardCount;
        ...
        >
          {ARTISTS.map((a, i) => {
            ...
            return (
              ...
              >
                <ArtistCard artist={a} cardAngle={cardAngle} parentAngle={angle} />
              </div>
            );
          })}
        </motion.div>
...
const ArtistCard = memo(function ArtistCard({
  ...
  artist: Artist;
  cardAngle: number;
  parentAngle: MotionValue<number>;
}) {
  const accent = artist.accent || '#FF4D00';

  // ⚡ Bolt: Use useTransform to compute visual state based on rotation without re-rendering the component
  const scale = useTransform(parentAngle, (angle) => {
  ...
  const boxShadow = useTransform(parentAngle, (angle) => {
    const relative = ((cardAngle + angle) % 360 + 360) % 360;
    const distFromFront = Math.min(relative, 360 - relative);
  ...
  const liveOpacity = useTransform(parentAngle, (angle) => {
    const relative = ((cardAngle + angle) % 360 + 360) % 360;
    const distFromFront = Math.min(relative, 360 - relative);
