 ...
 *
 * Optimized to use MotionValues and useAnimationFrame to prevent
 * high-frequency React re-renders.
 */
import { useRef, useState, memo } from 'react';
import { motion, useAnimationFrame, useMotionValue, useTransform, useMotionValueEvent, MotionValue } from 'motion/react';
...
export default function ArtistCarousel3D() {
  ...
  // Performance Optimization: Use motion/react's useAnimationFrame to update a MotionValue
  // instead of React state. This bypasses React's reconciliation loop for the 60fps rotation.
  ...
  // Track the active index for pagination dots, but only update React state when it changes
  useMotionValueEvent(angle, "change", (latest) => {
    // Optimization: Calculate front-most card index and update state only if it changed
    // to minimize pagination dot re-renders.
    let bestIndex = 0;
    let minDistance = 360;

    for (let i = 0; i < CARD_COUNT; i++) {
      const targetAngle = i * STEP_DEG;
      ...
      >
        ...
        >
          {ARTISTS.map((a, i) => (
            ...
            >
              <ArtistCard artist={a} totalAngle={angle} cardAngle={i * STEP_DEG} />
            </div>
          ))}
        </motion.div>
      ...
      <div className="flex items-center justify-center gap-2 mt-8 relative z-10">
        {ARTISTS.map((_, i) => (
          <button
            ...
            className={`h-1.5 transition-all ${i === activeIndex ? 'w-8 bg-primary' : 'w-1.5 bg-white/20 hover:bg-white/40'}`}
          />
        ))}
      </div>
 ...
 * ========================================================================= */
const ArtistCard = memo(({ artist, totalAngle, cardAngle }: { artist: Artist; totalAngle: MotionValue<number>; cardAngle: number }) => {
  const accent = artist.accent || '#FF4D00';

  // Calculate visual properties directly from the totalAngle MotionValue.
  // This avoids re-rendering the component at 60fps.
  const scale = useTransform(totalAngle, (latest) => {
    const relative = ((cardAngle + latest) % 360 + 360) % 360;
    const distFromFront = Math.min(relative, 360 - relative);
    return distFromFront < 30 ? 1.05 : 1;
  });

  const boxShadow = useTransform(totalAngle, (latest) => {
    const relative = ((cardAngle + latest) % 360 + 360) % 360;
    const distFromFront = Math.min(relative, 360 - relative);
  ...
  const liveBadgeOpacity = useTransform(totalAngle, (latest) => {
    const relative = ((cardAngle + latest) % 360 + 360) % 360;
    const distFromFront = Math.min(relative, 360 - relative);
